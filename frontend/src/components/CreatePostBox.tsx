  ...
  return (
    <div className="card" style={{ padding: 16, marginBottom: 20 }}>
      <div style={{ display: 'flex', gap: 12 }}>
        <div
          className="avatar"
          style={{ width: 40, height: 40, background: user?.avatarUrl ? undefined : avatarColorFor(user?.username || '?'), color: '#fff', cursor: 'pointer' }}
      ...
      {preview && (
        <div style={{ position: 'relative', marginTop: 12, maxHeight: 320, overflow: 'hidden', borderRadius: 12, display: 'flex', justifyContent: 'center', background: 'var(--bg-sunken)' }}>
          {file?.type.startsWith('video/') ? (
            <video src={preview} controls style={{ width: '100%', maxWidth: '100%', maxHeight: 320, height: 'auto', display: 'block', objectFit: 'contain' }} />
          ) : (
            <img src={preview} alt="" style={{ width: '100%', maxWidth: '100%', maxHeight: 320, height: 'auto', display: 'block', objectFit: 'contain' }} />
          )}
          <button
